...
import { Globe, Save, Loader2, Plus, Trash2, ExternalLink, Sliders, Bell, ChevronDown, ChevronUp, ChevronRight } from "lucide-react";
...
function PositionAccordion({ pos, onDelete, deleting, hasBorder }: {
  pos: PositionRow;
  onDelete: () => void;
  deleting: boolean;
  hasBorder: boolean;
  ...
  const dateRange = [formatDate(pos.startDate), pos.current ? "Present" : formatDate(pos.endDate)].filter(Boolean).join(" – ");

  return (
    <div style={{ borderBottom: hasBorder ? "1px solid var(--border)" : "none" }}>
      ...
      >
        <span style={{ color: "var(--text-subtle)", flexShrink: 0, display: "flex", transition: "transform 150ms", transform: open ? "rotate(90deg)" : "rotate(0deg)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        ...
        {positions.map((pos, i) => (
          <PositionAccordion
            ...
            deleting={deleting === pos.id}
            hasBorder={i < positions.length - 1}
          />
        ))}
