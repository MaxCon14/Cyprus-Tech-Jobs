...
export function LoginForm() {
  ...
  async function sendLink(targetEmail: string) {
    ...
    const check = await fetch("/api/candidates/check-email", {
      method: "POST",
    ...
    if (!exists) {
      setError("No candidate account found for that email.");
      setLoading(false);
      return;
    }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
        <div style={{ padding: "32px 28px", borderRadius: 14, border: "1px solid var(--border)", background: "var(--surface)", textAlign: "center", boxShadow: "var(--shadow-sm)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <button type="button" onClick={() => sendLink(email)} disabled={cooldown > 0} className="btn btn-outline" style={{ width: "100%", justifyContent: "center" }}>
              <RefreshCw size={13} />
              {cooldown > 0 ? `Resend in ${cooldown}s` : "Resend email"}
            </button>
            <button type="button" onClick={() => { setSent(false); setEmail(""); setError(null); }} className="btn btn-ghost btn-sm" style={{ width: "100%", justifyContent: "center", color: "var(--text-subtle)" }}>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ position: "relative" }}>
        ...
        <input className="input" type="email" required placeholder="you@email.com" value={email}
      ...
      <button type="submit" disabled={loading || cooldown > 0} className="btn btn-accent" style={{ width: "100%", justifyContent: "center" }}>
        {loading
