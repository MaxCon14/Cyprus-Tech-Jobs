...
export default async function CompaniesPage() {
  ...
  return (
    <div className="page-container" style={{ paddingBlock: "clamp(24px, 4vw, 40px)" }}>
      ...
      {/* CTA */}
      <div className="cta-strip" style={{ marginTop: 64, padding: "clamp(24px, 4vw, 40px)", border: "1px solid var(--border)", borderRadius: 12, background: "var(--surface-alt)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 20 }}>
        <div>
        ...
        <Link href="/post-a-job" className="btn btn-accent btn-lg" style={{ flexShrink: 0 }}>Post a job →</Link>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: large ? 16 : 12 }}>
        <div style={{ width: large ? 52 : 44, height: large ? 52 : 44, borderRadius: 8, background: "var(--black)", color: "var(--white)", display: "grid", placeItems: "center", fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: large ? 18 : 15, flexShrink: 0, overflow: "hidden" }}>
          {co.logoUrl
